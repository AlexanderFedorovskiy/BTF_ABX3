...
def cast_to_properties(data, labels):
    ...
    for row in data:
        ...
        for i in range(row.size - 1):
            prop_vector = []
            atom_number = normalize(row[i], 1.0, 100.0)  # up to Fermium n=100
            charge = get_charge(i, row[3])
            ionic_radii = normalize(get_ionic_radii(row[i], i, charge), 1.0, 221.0)  # Up to Te_raii=221
            ...
            cov_radii = normalize(get_cov_radii(row[i]), 1.0, 260.0) # up to Fr
            prop_vector.append(atom_number)
            prop_vector.append(charge)
            prop_vector.append(ionic_radii)
            prop_vector.append(elneg)
            prop_vector.append(cov_radii)
...
def get_ionic_radii(elementnumber, position, charge):
    ...
    #     return 143.0
        ...
        return 77.0  # Alternative value: 113, 110 (Li's ABX3 paper),  103 (new paper)
    elif elementnumber == 40 and position == 1 and charge == 2:  # Zr
        return 98.0  # From new paper
    elif elementnumber == 50 and position == 1 and charge == 2:  # Sn
        return 104.0  # From new paper
    elif elementnumber == 62 and position == 1 and charge == 2:  # Sm
        return 115.0  # From new paper
    elif elementnumber == 58 and position == 1 and charge == 2:  # Ce
        return 123.0  # From new paper
...
def makedata():
    test_data, test_labels = cast_to_properties(np.load("data\\ABO+ABX_test_data_v001.npy"),
                                                np.load("data\\ABO+ABX_test_labels_v001.npy"))
    training_data, training_labels = cast_to_properties(np.load("data\\ABO+ABX_training_data_v001.npy"),
                                                        np.load("data\\ABO+ABX_training_labels_v001.npy"))
    np.save('data\\ABO+ABX_training_data_v002+cov.npy', training_data)
    np.save('data\\ABO+ABX_training_labels_v002+cov.npy', training_labels)
    np.save('data\\ABO+ABX_test_data_v002+cov.npy', test_data)
    np.save('data\\ABO+ABX_test_labels_v002+cov.npy', test_labels)
    print("Done")
